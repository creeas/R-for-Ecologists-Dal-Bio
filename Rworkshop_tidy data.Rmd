...
### Strings
 ...
 Strings are vectors of characters. You can create strings with either single quotes or double quotes. When uploading data, columns that contain alpha numeric characters (EVEN IN A SINGLE CELL) will be traeted as a character, so always goodot check the class using `str()` or `dplyr::glimpse()`.
...
### Factors
 The `forcats` package has lots of tools for working with factors. And while I won't go into them here one of the most useful functions for controling factors is being able to control the order of them. This is really useful for models when you want to set one factor level as the baseline (ex. treatment) of if you want to plot factors in a certain order.
...
Data is often organized to make data entry easier, but this is not always ideal for analysis. This often results in two comon problems where one variable might be spread across multiple columns, spread across multiple rows, or column names may contain values. We can fix these common issues by pivoting using `pivot_longer` and `pivot_wider`.

## Pivot_longer

Common when dowloading large daasets including time-series are when columns names contain *values* of a variable. Take table4a: the column names 1999 and 2000 represent values of the year variable, the values in the 1999 and 2000 columns represent values of the cases variable, and each row represents two observations, not one. Here we need to move the column names to a column called `year` and the column values to a column called `cases`.
