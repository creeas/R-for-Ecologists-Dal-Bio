...
#NOTE: columns can also be selected by column number as in subset, but again it is best for reproducability to use explicit names in case your datframe changes.
```
...
data_batoids <- filter(data, class == "Batoids") #extract rows that pertain to a particlar group or variable level
...
`Summarise` is a function that will create a new database summarizing your data. This is useful for generating summary startistics based on an exisitng column. 
...
```{r}
new.data <- data %>% group_by(superorder) %>% mutate(mean.brain = mean(brain.mass)) %>%
...
When working within the tidyverse you can combine packages (dplyr and ggplot). Using this idea can you create a plot of the realtive difference in body mass over depth, with a different color for each superorder.
